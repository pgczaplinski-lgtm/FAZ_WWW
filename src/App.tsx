...
function App() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.querySelector<HTMLElement>('main h1, main')?.focus(), 0)
  }, [route])
  ...
  return <div className="site-shell">
    ...
    <header className="site-header">
      <a className="logo" href="/" onClick={navigate} aria-label="FAZ — strona główna"><strong>FAZ</strong><span>Fundacja Aktywności<br />Zawodowej</span></a>
...
function ProjectCard({ project, navigate }: { project: typeof projects[number]; navigate: (event: React.MouseEvent<HTMLAnchorElement>) => void }) { return <article className="project-card"><div className="project-top"><span className="status">{project.status === 'active' ? 'Trwa nabór' : 'Zakończony'}</span><span aria-hidden="true">↗</span></div><h3>{project.title}</h3><p>{project.lead}</p><a href={`/projekty/${project.slug}/`} onClick={navigate}>Dowiedz się więcej <span aria-hidden="true">→</span></a></article> }
...
function Story({ navigate }: { navigate: (event: React.MouseEvent<HTMLAnchorElement>) => void }) { return <section className="story section-wrap"><div className="story-visual" aria-hidden="true"><span>✦</span><strong>moja<br />droga</strong></div><div><p className="eyebrow">Głos uczestniczki</p><h2>„Najważniejsze było to, że ktoś <em>we mnie uwierzył.</em>”</h2><p className="lead">Historie uczestników pokazują, że odpowiednio dobrane wsparcie może pomóc odzyskać pewność siebie i zrobić kolejny krok w stronę pracy.</p><ButtonLink href="/projekty/" className="text-link" onClick={navigate}>Znajdź swoją drogę <span aria-hidden="true">→</span></ButtonLink></div></section> }
...
function About({ navigate }: { navigate: (event: React.MouseEvent<HTMLAnchorElement>) => void }) { return <><PageIntro eyebrow="Poznajmy się" title="O nas" text="Od 2008 roku tworzymy warunki, w których każdy może rozwijać swój potencjał." ><div className="about-mission"><h2>Nasza misja</h2><p>FAZ wspiera przede wszystkim osoby z niepełnosprawnościami w rozwoju zawodowym, rehabilitacji oraz pełniejszym uczestnictwie społecznym. Organizujemy giełdy pracy, doradztwo zawodowe i prawne, a także szkolenia dla pracodawców i WTZ.</p><p>Jesteśmy organizacją pożytku publicznego i działamy non-profit.</p></div></PageIntro><section className="about-stats section-wrap"><p className="eyebrow">Od 2008 roku</p><div className="stats">{impactSince2008.map(item => <div className="stat" key={item.label}><strong>{item.value}</strong><span>{item.label}</span></div>)}</div></section><section className="timeline section-wrap"><SectionIntro eyebrow="Nasza historia" title="Krok po kroku, od 2008 do dziś" /><div className="timeline-line"><div><b>2008</b><p>Początek działalności Fundacji Aktywności Zawodowej.</p></div><div><b>Dziś</b><p>Łączymy doświadczenie z nowymi sposobami wspierania osób z niepełnosprawnościami.</p></div></div></section><section className="team section-wrap"><SectionIntro eyebrow="Ludzie FAZ" title="Zespół, który działa razem" /><div className="team-grid">{team.map(person => <div className="team-card" key={person.name}><div className="avatar" aria-hidden="true">{person.name.slice(0, 1)}</div><h3>{person.name}</h3><p>{person.role}</p></div>)}</div></section><ContactCta navigate={navigate} /></> }
